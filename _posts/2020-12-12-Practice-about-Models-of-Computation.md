...
Language generated is $$L = \{a,b\}^*$$, where is has length greater than 1.
...
First,  we show that G only generates strings in L. The first rules rewrite S until S is removed from the intermediate string of the form $$uaTbv$$ or $$ubTav$$, where $$\mid u \mid = \mid v \mid$$. The rules second rule will generate the form $$x\#y$$, where $$\mid x \mid = \mid y \mid$$. Applying those rules in any order we get the derivation is strings S of the form $$uax\#ybv$$ and $$ubx\#yav$$, where $$\mid u\mid = \mid v\mid$$ and $$\mid x \mid = \mid y \mid$$, where $$S \in L$$.

Second, we show that G generates all strings in L. A string s is in L if and only if (i.e. exactly when) it can be written as $$uax\#ybv$$ or $$ubx\#yav$$ for some u, v, x, y with $$\mid x \mid = \mid y \mid$$ and $$\mid x \mid = \mid y \mid$$ (to see this simply let $$uk$$ be the shortest preﬁx of s such that $$k \in \{a,b\}$$ and the reverse of uk is not a suﬃx of s. But we have seen that each such string is generated by G.
